...
class LinearVisualizer:
    ...
    def scatter_yy(self, ref='line', annotate_threshold=0.1, figure_name=None, **kwargs):
        ...
        if ref is not None:
            ...
            if ref == 'line':
                ...
                plt.plot(yrange, yrange, '-', c='b', lw=0.4)

        groupnames = list(self.groups.keys())

        plots = [
            plt.scatter(self.groups[l][0], self.groups[l][1], c=LinearVisualizer.colormap[i], **kwargs) for i, l in enumerate(groupnames)
            ]
    ...
    def scatter_error(self, ref='line', error='abs', annotate_threshold=0.1, figure_name=None, **kwargs):
        ...
        plots = [
            plt.scatter(self.groups[l][0], error_func(self.groups[l][0], self.groups[l][1]), c=LinearVisualizer.colormap[i], **kwargs) for i, l in enumerate(groupnames)
            ]
        plt.legend(plots, groupnames, loc='lower right')
        ...
        if ref is not None:
            ...
            if ref == 'line':
                ...
                plt.axhline(0, yrange[0], yrange[1], c='b', lw=0.4)
